...
def test_snapshot_threads_empty(tmp_path: Path):
    ...
    assert snapshot_threads(str(tmp_path)) == {}


def test_snapshot_threads_captures_files(tmp_path: Path):
    """Each *.jsonl file produces a (mtime, size) entry keyed by stem."""
    cwd = str(tmp_path)
    t1 = create_thread(cwd, participants=["a", "b"])
    t2 = create_thread(cwd, participants=["b", "c"])

    snap = snapshot_threads(cwd)
...
def test_snapshot_threads_ignores_index_json(tmp_path: Path):
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["a"])
    snap = snapshot_threads(cwd)
    assert tid in snap
...
def test_check_thread_changes_first_scan_no_notifications(
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["pm", "tl"])
    append_message(cwd, tid, sender="pm", content="hello")

    writes = _install_fd_capture(monkeypatch)
    new_snap = check_thread_changes(cwd, "tl", tmp_path / "in", {})
...
def test_check_thread_changes_notifies_participant(
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["pm", "tl"])
    prev = snapshot_threads(cwd)

    append_message(cwd, tid, sender="pm", content="hello tl")

    writes = _install_fd_capture(monkeypatch)
    new_snap = check_thread_changes(cwd, "tl", tmp_path / "in", prev)
...
def test_check_thread_changes_ignores_own_message(
    ...
    check_thread_changes(cwd, "pm", tmp_path / "in", prev)
...
def test_check_thread_changes_ignores_non_participant(
    ...
    writes = _install_fd_capture(monkeypatch)
    check_thread_changes(cwd, "rhc", tmp_path / "in", prev)
...
def test_check_thread_changes_preview_truncation(
    ...
    append_message(cwd, tid, sender="pm", content=long_content)

    writes = _install_fd_capture(monkeypatch)
    check_thread_changes(cwd, "tl", tmp_path / "in", prev)
...
def test_check_thread_changes_no_truncation_at_exact_length(
    ...
    append_message(cwd, tid, sender="pm", content=exact_content)
...
def test_check_thread_changes_empty_worker_name(
    ...
    writes = _install_fd_capture(monkeypatch)
    new_snap = check_thread_changes(cwd, "", tmp_path / "in", prev)
...
def test_check_thread_changes_multiple_new_messages(
    ...
    append_message(cwd, tid, sender="pm", content="first")
    append_message(cwd, tid, sender="pm", content="second")
    append_message(cwd, tid, sender="pm", content="third")
...
def test_check_thread_changes_mixed_senders(
    ...
    append_message(cwd, tid, sender="pm", content="from pm")
    append_message(cwd, tid, sender="tl", content="from tl self")
    append_message(cwd, tid, sender="pm", content="from pm again")
...
def test_check_thread_changes_no_growth_no_notification(
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["pm", "tl"])
    append_message(cwd, tid, sender="pm", content="already here")
    prev = snapshot_threads(cwd)
...
def test_read_new_messages_since_size_partial(tmp_path: Path):
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["a"])
    append_message(cwd, tid, sender="a", content="one")
    append_message(cwd, tid, sender="a", content="two")

    snap = snapshot_threads(cwd)
    size_after_two = snap[tid][1]

    append_message(cwd, tid, sender="a", content="three")

    new_msgs = _read_new_messages_since_size(cwd, tid, size_after_two)
...
def test_read_new_messages_since_size_zero_reads_all(tmp_path: Path):
    ...
    new_msgs = _read_new_messages_since_size(cwd, tid, 0)
...
def test_read_new_messages_since_size_missing_thread(tmp_path: Path):
    ...
    assert _read_new_messages_since_size(str(tmp_path), "nonexistent", 0) == []
...
def test_read_new_messages_since_size_skips_corrupt_lines(tmp_path: Path):
    ...
    cwd = str(tmp_path)
    tid = create_thread(cwd, participants=["a"])
    thread_file = Path(cwd) / ".cwork" / "threads" / f"{tid}.jsonl"
